...
- **Provider uncertainty is stated, not guessed.** If the provider was called but the result could not be confirmed or persisted, the claim is parked as `PROVIDER_RESULT_UNKNOWN` and never retried automatically, because a retry could duplicate a real Payment Link. A known provider rejection remains an explicit `FAILED` outcome and stays retryable exactly as in Phase 11.
