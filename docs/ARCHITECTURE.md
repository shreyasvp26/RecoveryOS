...
**Phase 8 delivered the evaluation boundary; Phase 9 the honest benchmark; Phase 10 the read-only Command Center & Decision Trace; Phase 11 real Razorpay Payment Link execution; Phase 12 the closed-loop verified webhook; Phase 13 adversarial policy/trace hardening; Phase 14 end-to-end live verification, evidence, and documentation of V1; Phase 16 the V2 economic optimizer; Phase 17 the signal-bearing benchmark; Phase 18 the economic decision audit trail; Phase 19 the Policy Lab; Phase 20 the revenue-degradation detector; Phase 21 the Recovery Operations Center; Phase 22 recovery-intelligence feedback; Phase 23 the evidence-calibrated adaptive estimator; Phase 24 final end-to-end recovery-loop verification and submission readiness.** This repository currently contains:
...
- A React + Vite operator frontend that renders persisted backend state and never recomputes policy or benchmark metrics. It began as three read-only screens (Command Center, Event Decision Trace, Policy & Blocks); Phase 19 added the Policy Lab, Phase 20 the Revenue Health screen, Phase 21 the Recovery Operations Center — the one screen that can request an action, which the server then decides and performs on its own authority — Phase 22 the Recovery Intelligence view, and Phase 23 the Estimator Evidence screen.

The V1 pipeline is fully implemented through Phase 12: the Phase 9 benchmark runs on top of the Phase 8 evaluation foundation (hidden model + simulation); Phase 10 added the read-only Recovery Command Center & Decision Trace over persisted state; Phase 11 added real Razorpay Test Mode Payment Link execution; and Phase 12 added the closed-loop verified webhook outcome channel (documented below). The V2 economic optimizer arrived in Phase 16 and is the production selection strategy today; the Phase 23 adaptive estimator recalibrates only the probability used to rank, never what is authorized or executed; and the Phase 9 benchmark predates both and measures the V1 arm only (see *The V2 economic optimizer (Phase 16)* and *The Phase 17 signal-bearing benchmark* below). The benchmark measures **No Action**, **Naive Retry**, and the real **RecoveryOS** pipeline over ONE shared event set and ONE shared hidden outcome model, on simulated, labeled recovery outcome amounts, reporting honest results with no forced RecoveryOS victory. The outcome engine answers *did an executed intervention recover the money?* It never decides whether anything executes, and RecoveryOS claims no real revenue.

Phase 14 verified this pipeline end to end against real Razorpay **Test Mode** infrastructure: a real classification, a deterministic authorization and selection, a real Test Mode Payment Link, a real manual payment, and a genuine `payment_link.paid` webhook that was HMAC-verified, correlated to the persisted link, and recorded as a single trusted recovery. **Test Mode is not production payment processing, and no production readiness is claimed.**

Phase 24 closes the submission-readiness loop on top of that verified chain: a single end-to-end golden-path integration test (ingest → classify → V2 selection → real Test Mode authorization/execution → `waiting` trace → verified `payment_link.paid` webhook → `recovered` trace → recovery-intelligence observation) plus API-level safety-scenario coverage (per-customer retry limit denial, daily spend-cap blockage, calibration rank-change with immutable history, and `threshold_not_met` observability), and all of it — backend suite and frontend lint/build — is enforced by GitHub Actions CI. These tests exercise the same public execution, recovery, trace, webhook, and intelligence endpoints, so they assert the loop as the operator sees it. See `backend/tests/test_phase24_end_to_end.py`. Two limitations remain load-bearing for interpreting any demo: the external LLM classification can vary between equivalent events even at `temperature = 0` (the deterministic policy, selector, and optimizer are reproducible only *given* a classifier output), and the **Phase 9** benchmark's hidden outcome model carries no signal — only the separate, current Phase 17 signal-bearing benchmark can demonstrate the value of targeting. See below and `docs/BENCHMARK.md`.
...
- **Honest denominators and integer money.** Recovery rate reuses the canonical definition (recovered ÷ *scored* events) in integer basis points; events with no evaluated outcome are excluded rather than counted as misses. The classic failure rate (failed ÷ total) is published too, and published as inert: RecoveryOS ingests only already-failed payments, so it is 100% in every window and every segment by construction and is never an input to detection. Status follows the same discipline — current detections are `OPEN`, and `RESOLVED` exists only as the pure reconciliation of a previously observed set against a newer one, because Phase 20 keeps no incident history to claim otherwise. Simulated revenue at risk is `max(0, degradation) × current-window payment value ÷ 10,000` in integer paise — a **modelled** estimate, labelled as such everywhere, never production revenue or merchant loss.
...
### The Phase 21 Recovery Operations Center

Phases 1–20 answer *what should RecoveryOS do?* and *where is recovery degrading?* Phase 21 adds the operational layer between them — *which failed payments need attention right now, and what happened to them?* — without adding a second decision path.

- **The queue is a projection, not a source of truth.** `app/recovery_operations.py` derives every row from records the existing pipeline already persists: `payment_events`, `classification_results`, `policy_decisions`, `optimizer_decisions`, `execution_outcomes` and `webhook_recovery_outcomes`. There is no `recovery_queue` table and no second lifecycle store, so a row can never disagree with the authoritative records. `build_queue_row` is pure, so the same records always produce the same row.
- **Execution is not recovery.** A `REAL_RAZORPAY` Payment Link that was created successfully is `PENDING_OUTCOME`. It becomes `RECOVERED` only when the Phase 12 webhook path has persisted a verified, correlated recovery for that exact `payment_link_id`, and the reported amount is the trusted `amount_paid` the provider observed — never the original event amount.
- **Simulated is not real.** A `SIMULATED` intervention reaches `EXECUTED` and stops there. It never carries a recovered amount, because no provider was contacted, and every actionable row states its mode so a viewer cannot mistake one for the other.
- **The operator chooses whether, never what.** `POST /recovery/{event_id}/execute` reuses `execution_service.execute_event`: the classification, the deterministic policy decisions, the economic selection and the bounded execution are all re-derived from authoritative server state. A request that supplies an `intervention`, `allowed`, `policy_decision`, `execution_mode` or `evaluation_time` is refused with 422 rather than quietly ignored, so a client can never believe it influenced the decision.
- **One durable claim closes the concurrency gap.** The Phase 6 gate blocks *sequential* duplicates by reading persisted history, but two simultaneous requests can both read that history before either writes its attempt: both are genuinely authorized and both would reach the provider. `execution_claims` makes `(event_id, intervention)` a PRIMARY KEY, taken immediately before the executor and resolved immediately after, so SQLite decides which single attempt crosses the side-effect boundary. It is a concurrency primitive, not authorization — holding a claim confers no permission and cannot make a denied candidate executable.
- **Provider uncertainty is stated, not guessed.** The Razorpay boundary separates a proven refusal (Razorpay read the request and rejected it; nothing exists) from a result RecoveryOS cannot determine (timeout, connection reset, 5xx, an unreadable response, or a failure between the side effect and its record). A known failure keeps the Phase 11 semantics exactly: explicit `FAILED`, claim released, still retryable. An unknown result is also recorded as `FAILED` — RecoveryOS observed no success and claims none — but the claim is **retained** as `PROVIDER_RESULT_UNKNOWN`, the row is not actionable, and no further attempt is permitted, because a retry could create a second real Payment Link. Nothing is retried automatically anywhere in this path.

Exposed additively as `GET /recovery/queue` and `POST /recovery/{event_id}/execute`, and rendered by the **Recovery Operations Center** screen, which drills into the existing `GET /events/{id}/trace` rather than growing a second event view. See `docs/RECOVERY_OPERATIONS.md` for the state table, the safety invariants and the limitations.
...
### The Phase 23 adaptive recovery estimator

Phase 23 evolves the Phase 16 V2 economics from a **static additive score** into **evidence-calibrated economics**, additively and without touching the frozen decision chain.

- **A calibration layer, not a new authority.** `app/calibration.py` owns the terminal contract (provider status → calibration outcome), the integer-exact Beta-binomial posterior against a baseline-derived prior, and the per-intervention evidence gate (`observed_total >= 10, recovered >= 1, not_recovered >= 1`). It executes nothing, authorizes nothing, and imports no executor, policy engine, optimizer, webhook boundary, classifier, or benchmark.
- **The adaptive estimator composes, additively.** `CalibratedRecoveryProbabilityEstimator` (`app/adaptive_estimation.py`) wraps the frozen baseline with an immutable `CalibrationSnapshot`; for a gated intervention it returns the calibrated posterior, otherwise the baseline. Its contract (`estimate -> RecoveryProbability`) is unchanged, so the optimizer consumes it identically. `select_for_strategy` and `execute_event` accept an optional `estimator` that defaults to the frozen baseline — the V1/V2 benchmark and Policy Replay arms reproduce their recorded results exactly, and only the production execute endpoint constructs the calibrated wrapper when an active gated snapshot exists.
- **Evidence is real operational world only.** Calibration is fed exclusively by `REAL_RAZORPAY` `payment_link` terminal outcomes: verified webhook recoveries (positive) plus provider-confirmed `expired` (negative) observed via an additive read-only `get_payment_link` on the Razorpay boundary. PENDING/`created`/`partially_paid` and `UNKNOWN` (incl. `cancelled`, failures, timeouts) are never samples, and `NOT_RECOVERED` is never inferred from absence. SIMULATED, benchmark, Policy Lab, replay and hidden ground truth are structurally ineligible.
- **Snapshots are immutable and versioned.** `app/calibration_service.py` projects the evidence and appends exactly one row per build to `estimator_calibration_snapshots` (version is the PK; duplicates rejected; no update/delete path), so past estimates and the decisions they preceded stay reconstructable and are never rewritten. An operator triggers a build explicitly via `POST /estimator-evidence/recalibrate`; nothing recalibrates on its own. `GET /estimator-evidence` reports the current/historical state and whether calibration is active.

Exposed additively and rendered by the **Estimator Evidence** screen. Policy remains the authorization boundary and the optimizer remains the decision layer; the estimator only changes the probability used to rank, never what is authorized or executed. See `docs/ADAPTIVE_ESTIMATION.md` for the terminal contract, the gate, the arithmetic and the stated limitations.
